...
class ClientConnection implements AcpClient {
  ...
  async createTerminal(params: CreateTerminalRequest): Promise<TerminalHandle> {
    const { terminalId } = await this.ctx.request(methods.client.terminal.create, params);
    const terminalParams = { sessionId: params.sessionId, terminalId };
    const release = () => this.ctx.request(methods.client.terminal.release, terminalParams);
    return {
      id: terminalId,
      currentOutput: () => this.ctx.request(methods.client.terminal.output, terminalParams),
      waitForExit: () => this.ctx.request(methods.client.terminal.waitForExit, terminalParams),
      kill: () => this.ctx.request(methods.client.terminal.kill, terminalParams),
      release,
...
export class ClaudeAcpAgent {
  ...
  private async createSession(
    ...
    // If the client supports ACP `terminal/*` methods, route shell execution
    // through it instead of running Bash locally in the agent process. This
    ...
    // When ACP terminal routing is active, suppress the built-in shell tools
    // so the model has no choice but to use the MCP-backed `bash` replacement.
    // BashOutput polls a background shell that the SDK Bash tool would have
    // started; without Bash it cannot work anyway.
    if (supportsAcpTerminal) {
      disallowedTools.push("Bash", "BashOutput");
    }
      ...
      case "mcp_tool_use": {
        ...
        if (chunk.name === "TodoWrite") {
          ...
          try {
            ...
            // Ignore inputs that cannot be represented as JSON.
          }
