...
function injectAcpTerminalPermissionSession(agent: ClaudeAcpAgent): void {
  function* empty() {}
  const query = Object.assign(empty(), { interrupt: vi.fn(), close: vi.fn() });
  agent.sessions["acp-terminal-session"] = {
    query,
    input: new Pushable(),
    cancelled: false,
    cwd: "/test",
...
describe("ACP terminal permission FIFO", () => {
  function createAgent(decision: "allow" | "reject") {
    const client = {
      sessionUpdate: async () => {},
      requestPermission: async () => ({
        outcome: {
    ...
    const agent = new ClaudeAcpAgent(client, { log: () => {}, error: () => {} });
    injectAcpTerminalPermissionSession(agent);
    return agent;
  }

  it("queues allowed ACP terminal tool ids in permission order", async () => {
    const agent = createAgent("allow");
    const canUseTool = agent.canUseTool("acp-terminal-session");
    for (const toolUseID of ["tool-one", "tool-two"]) {
      await canUseTool(ACP_TERMINAL_TOOL_NAME, { command: "echo ok" }, {
        signal: new AbortController().signal,
        toolUseID,
      } as any);
    }
    expect(agent.sessions["acp-terminal-session"].pendingAcpTerminalToolUses).toEqual([
      "tool-one",
      "tool-two",
    ]);
  });

  it("does not queue denied or non-ACP tool ids", async () => {
    const denied = createAgent("reject");
    await denied.canUseTool("acp-terminal-session")(
      ACP_TERMINAL_TOOL_NAME,
      { command: "echo no" },
      { signal: new AbortController().signal, toolUseID: "denied" } as any,
    );
    expect(denied.sessions["acp-terminal-session"].pendingAcpTerminalToolUses).toEqual([]);

    const allowed = createAgent("allow");
    await allowed.canUseTool("acp-terminal-session")("Bash", { command: "echo local" }, {
      signal: new AbortController().signal,
      toolUseID: "local",
    } as any);
    expect(allowed.sessions["acp-terminal-session"].pendingAcpTerminalToolUses).toEqual([]);
  });
});
